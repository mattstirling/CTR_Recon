...
def apply_map_rule(value,rulename):
    ...
    try:
        ...
        elif rulename == 'WSS_VAR_right19_if_rec':return WSS_VAR_right19_if_rec(value)
        elif rulename == 'WSS_remove_CTR_prefix_and_add_Rec':return WSS_remove_CTR_prefix_and_add_Rec(value)
        else: return 'rule not in if-else tree'
    except:
        return value

def WSS_remove_CTR_prefix_and_add_Rec(value):
    match = re.search(r'.*[.]([0-9]*)',value)
    if match: return match.group(1) + ' Rec'
    else: return value

def WSS_VAR_right19_if_rec(value):
    match = re.search(r'.*([0-9]{13}) [0-9]{15}( Rec)',value)
