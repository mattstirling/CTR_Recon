...
in_file = 'K2_Credit_Total_Return_Swap_D_20170131_16_RiskWatch.csv'
out_file = in_file.replace('.csv','_v10.csv')
...
df_merge['Asset Resets_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['Asset Resets_bench']]
df['Asset Resets'] = df_merge['Asset Resets_bench'] 

df_merge['FI Applied Period Dates_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['FI Applied Period Dates_bench']]
df['FI Applied Period Dates'] = df_merge['FI Applied Period Dates_bench'] 

df_merge['Notional Adj Asset Resets_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['Notional Adj Asset Resets_bench']]
df['Notional Adj Asset Resets'] = df_merge['Notional Adj Asset Resets'] 

df_merge['Notional Adj Rate Resets_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['Notional Adj Rate Resets_bench']]
df['Notional Adj Rate Resets'] = df_merge['Notional Adj Rate Resets_bench'] 

df_merge['Notional Adjustments_bench'] = [preprocess_ETL.list_bar_to_comma_space_with_brackets_always(i) for i in df_merge['Notional Adjustments_bench']]
#df_merge['Notional Adjustments_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['Notional Adjustments_bench']]
df['Notional Adjustments'] = df_merge['Notional Adjustments_bench'] 

df_merge['FI Spread_bench'] = [preprocess_ETL.date_list_bar_to_comma_space(i) for i in df_merge['FI Spread_bench']]
df['FI Spread'] = df_merge['FI Spread_bench'] 

df['Maturity Date'] = df_merge['Maturity Date_bench'] 
#df['Underlying Instrument'] = df_merge['Underlying Instrument_bench'] 
